...
public class ListUsers : IListUsers
{
    ...
    public ListUsers(IUserRepository userRepository) => _userRepository = userRepository;
    ...
    )
    {
        ...
        var searchOutput = await _userRepository.Search(searchInput, cancellationToken);

        return await ToOutput(searchOutput);
    }

    private Task<ListUsersOutput> ToOutput(SearchOutput<Domain.Entity.User> searchOutput)
    {
