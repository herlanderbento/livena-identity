...
public class RevokedTokenRepository(LivenaIdentityDbContext context): IRevokedTokenRepository
{
    ...
    {
        var model = await RevokedTokens.AsNoTracking().FirstOrDefaultAsync(
            x => x.Id == id, 
            cancellationToken
        );
    ...
    public async Task<SearchOutput<RevokedToken>> Search(SearchInput<string> input, CancellationToken cancellationToken)
    {
